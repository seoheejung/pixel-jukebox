...
function youtubeSearchBody(recommendations: Recommendation[], individual = false): Record<string, unknown> {
  const strategy = individual
    ? 'Run a focused exact-match search with the artist and song title quoted plus "official MV", "official music video", or "공식 뮤직비디오". Inspect results for the full music video on an official artist or label channel. Search-result, channel, playlist, and category pages are unusable; cite a direct video page, or return no URL if no supported MV exists.'
    : 'Search for an official full-length music video (MV) for every supplied song. Include "official MV" or "official music video" in each artist-and-title query. Return one best supported MV per song.';
  return {
    model: 'gpt-4.1-mini', store: false,
    tools: [{ type: 'web_search' }], tool_choice: 'required', include: ['web_search_call.action.sources'],
    input: [
      { role: 'developer', content: `Find the official full-length music video (MV) for ${individual ? 'the supplied artist and song' : 'each supplied artist and song'}. ${strategy} Search site:youtube.com and site:youtu.be using supplied names and any Korean, English, or romanized alias explicitly supported by evidence. Rank eligible results by uploader authority: (1) the official artist channel, then (2) the verified label or distributor channel that released the original MV. Choose the highest-priority supported result. Do not substitute live stages, fancams, covers, lyric videos, audio-only uploads, Shorts, or teaser clips. If no supported MV is found, return no URL so another candidate can be tried. Return direct YouTube video URLs with citations, retaining the associated artist and song. Do not invent, translate, or guess names or links.` },
